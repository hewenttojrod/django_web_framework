...
set "WORKSPACE_DIR=%CD%\web_core\workspace"
set "WEB_ID="
set "APP_COUNT=0"
set "SELECTED_APPS="
set "MIGRATE_ALL="
...
if !APP_COUNT! EQU 0 (
    echo [ERROR] No Django workspace apps were found under "%WORKSPACE_DIR%".
    popd
    exit /b 1
)
...
:run_manage
if defined WEB_ID (
    docker compose -f "%COMPOSE_FILE%" exec -T web python manage.py %*
) else (
    docker compose -f "%COMPOSE_FILE%" run --rm web python manage.py %*
)
exit /b %errorlevel%
